...
@section('title', 'Modifier : ' . $menu->intitule)
@section('page-title', 'Modifier le plat')
...
<style>
    ...
    .prix-wrap .field-input { padding-right: 56px; }
    ...
    .modif-row:last-child { border-bottom: none; }
</style>
@endpush
...
══════════════════════════════════════════════════════════ --}}
<form method="POST"
      action="{{ route('admin.menus.update', $menu->idmenu) }}"
                ...
                <div class="card-body">
                    ...
                    <div class="field-group">
                        ...
                        <select name="idcategorie"
                            ...
                            <option value="{{ $cat->idcategorie }}"
                                {{ old('idcategorie', $menu->idcategorie) == $cat->idcategorie ? 'selected' : '' }}>
                                {{ $cat->intitule }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="field-group">
                        ...
                        <input type="text"
                               ...
                               value="{{ old('intitule', $menu->intitule) }}"
                               placeholder="Ex: Poulet braisé, Ndolé..."
                               maxlength="128"
                               autocomplete="off"
                               oninput="
                                   validerIntitule();
                    ...
                    <div class="field-group">
                        ...
                        <textarea name="description"
                                  ...
                                  oninput="compterChars('description','cnt_description',1000)">{{ old('description', $menu->description) }}</textarea>
                    ...
                    <div class="field-group">
                        ...
                        {{-- Aperçu prix --}}
                        <div class="prix-preview" id="prixPreview">
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="photo-zone"
                        ...
                        {{-- Photo existante ou aperçu nouvelle photo --}}
                        <img id="photoPreview"
                             src="{{ $menu->photo ? asset('storage/' . $menu->photo) : '' }}"
                             alt="{{ $menu->intitule }}"
                             style="{{ $menu->photo ? '' : 'display:none;' }}">

                        {{-- Placeholder (visible si pas de photo) --}}
                        <div class="photo-zone-placeholder"
                            ...
                            <div style="font-size:12px;color:#444;font-weight:500;">
                                {{ $menu->photo ? 'Cliquez pour remplacer la photo' : 'Cliquez ou glissez une image' }}
                            </div>
                    ...
                    {{-- Info nouvelle photo sélectionnée --}}
                    <div id="photoInfo"
                        ...
                        <button type="button"
                                ...
                                title="Annuler la nouvelle photo">
                            <i class="fa-solid fa-xmark"></i>
                        </button>
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="toggle-field" onclick="basculerStatut()">
                        <div>
                            <div style="font-size:13px;font-weight:600;"
                                 id="statutLabel">
                                {{ old('statut', $menu->statut) }}
                            </div>
                            <div style="font-size:11px;color:#444;margin-top:2px;"
                                 id="statutDesc">
                                {{ old('statut', $menu->statut) === 'Activé'
                                    ? 'Visible dans les commandes'
                                    : 'Masqué dans les commandes' }}
                            </div>
                        </div>
                        <div class="toggle-switch {{ old('statut', $menu->statut) === 'Activé' ? 'on' : 'off' }}"
                             id="toggleSwitch"></div>
                    </div>
                    ...
                    <div style="margin-top:12px;padding:10px 12px;border-radius:8px;
                        ...
                        @if(old('statut', $menu->statut) === 'Activé')
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:16px 20px;">
                    <div class="modif-row">
                        <span style="color:#444;">Référence</span>
                        <span style="color:#666;font-family:monospace;">
                            #{{ $menu->idmenu }}
                        </span>
                    </div>
                    <div class="modif-row">
                        <span style="color:#444;">Créé le</span>
                        <span style="color:#666;">
                            {{ $menu->created_at->format('d/m/Y') }}
                        </span>
                    </div>
                    <div class="modif-row">
                        <span style="color:#444;">Dernière modification</span>
                        <span style="color:#666;">
                            {{ $menu->updated_at->format('d/m/Y à H:i') }}
                        </span>
                    </div>
                    <div class="modif-row">
                        <span style="color:#444;">Catégorie actuelle</span>
                        <span style="color:#60a5fa;font-weight:600;">
                            {{ $menu->categorie->intitule ?? '—' }}
                        </span>
                    </div>
                    @if($menu->lignes_count ?? 0 > 0)
                    <div class="modif-row" style="border-bottom:none;">
                        <span style="color:#444;">Fois commandé</span>
            ...
            @if($menu->photo)
            <form method="POST"
                  action="{{ route('admin.menus.supprimer-photo', $menu->idmenu) }}"
                  id="deletePhotoForm"
                  style="display:none;">
                @csrf @method('DELETE')
            </form>
...
document.addEventListener('DOMContentLoaded', () => {
    // Initialiser les compteurs avec les valeurs actuelles
    compterChars('intitule',    'cnt_intitule',    128);
    compterChars('description', 'cnt_description', 1000);

    // Mettre à jour l'aperçu du prix avec la valeur actuelle
    majApercu();

    // Initialiser la couleur du label de statut
    const statut = document.getElementById('inputStatut').value;
    majCouleurStatut(statut);
});
...
function appliquerStatut(statut) {
    ...
    input.value = statut;
    toggle.className = `toggle-switch ${statut === 'Activé' ? 'on' : 'off'}`;

    majCouleurStatut(statut);

    document.getElementById('statutDesc').textContent =
        statut === 'Activé'
            ? 'Visible dans les commandes'
            : 'Masqué dans les commandes';

    info.innerHTML = statut === 'Activé'
        ? `<i class="fa-solid fa-circle-info" style="color:#444;margin-right:5px;"></i>
           Un plat <strong style="color:#22c55e;">Activé</strong>
           apparaîtra dans le formulaire de prise de commande.`
        : `<i class="fa-solid fa-circle-info" style="color:#444;margin-right:5px;"></i>
           Un plat <strong style="color:#f87171;">Désactivé</strong>
...
function majCouleurStatut(statut) {
    ...
    label.style.color = statut === 'Activé' ? '#22c55e' : '#f87171';
}
...
/**
 * Annuler la nouvelle photo sélectionnée
 * et restaurer l'affichage de la photo actuelle
 */
function annulerNouvellePhoto() {
    ...
    phld.style.display = 'none';
    @else
    // Pas de photo existante : afficher le placeholder
...
/**
 * Supprimer la photo enregistrée sur le serveur
 * via une requête POST (formulaire caché)
 */
...
document.getElementById('menuForm').addEventListener('submit', function(e) {
    ...
    // Confirmation avant soumission si des modifications existent
